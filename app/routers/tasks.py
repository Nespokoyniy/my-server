...
def get_tasks(db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user_id = verify_token(token)
    tasks = tasks.get_tasks(user_id, db)
    return tasks
...
def update_task(body: schemas.Task, task_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user_id = verify_token(token)
    body = schemas.TaskWithOwner(**body.model_dump(), owner=user_id)
    updated_task = db_exc_check(tasks.update_task, {"task_id": task_id, "body": body, "db": db})
...
def delete_task(task_id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    user_id = verify_token(token)
    task = db_exc_check(tasks.delete_task, {"user_id": user_id, "task_id": task_id, "db": db})
